...
namespace MonoCross.Utilities.Storage
{
    ...
    public class BasicFile : IFile
    {
        ...
        public virtual byte[] Read(string filename, string key, byte[] salt)
        {
            ...
            Device.Log.Metric(string.Format("BasicFile.Read(key,salt): File: {0} Time: {1:0} milliseconds", filename, DateTime.UtcNow.Subtract(dtMetric).TotalMilliseconds));
        ...
        public virtual string[] GetFileNames(string directoryName)
        {
        ...
        public virtual string[] GetDirectoryNames(string directoryName)
        {
        ...
        protected virtual void SaveClear(string filename, string contents)
        {
            ...
            EnsureDirectoryExistsForFile(filename);
            var stream = new MemoryStream(Encoding.Unicode.GetBytes(contents));
